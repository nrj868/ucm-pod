...
#   bash scripts/run_dramstore_hixl_single.sh
#   POOL_DEVICE=4 CLIENT_DEVICE=5 REQUEST_SIZE=4 bash scripts/run_dramstore_hixl_single.sh
#
...
REQUEST_SIZE="${REQUEST_SIZE:-4}"          # blocks per dump; 4 verified on A2
...
nohup "$STAGE_DIR/drampool" \
    ...
    --transport-protocol hixl --pool-size-mb "$POOL_MB" \
...
python3 "$STAGE_DIR/dramstore_bench_test.py" \
    --transport-protocol hixl --so-dir "$STAGE_DIR" \
    --kv-cache-memory-type device \
    --local-host 127.0.0.1 --ibverbs-device "" --transport-device-id "$CLIENT_DEVICE" \
    --node-ids 1 \
    ...
    --layer-size 1 --chunk-size 1 --request-size "$REQUEST_SIZE" \
